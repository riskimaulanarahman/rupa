...
@section('page-title', __('package.edit'))

@section('content')
<div class="max-w-4xl mx-auto">
    <!-- Back Button -->
    <a href="{{ route('packages.show', $package) }}" class="inline-flex items-center text-sm max-sm:text-xs text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300 mb-6 max-sm:mb-4">
    ...
    <form action="{{ route('packages.update', $package) }}" method="POST" class="space-y-6 max-sm:space-y-4">
        @csrf
        @method('PUT')
        ...
        <!-- Pricing -->
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 max-sm:p-4" x-data="packagePricing({{ old('original_price', $package->original_price) }}, {{ old('package_price', $package->package_price) }})">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 max-sm:p-4">
            <label class="flex items-center gap-3 cursor-pointer">
                <input type="checkbox" name="is_active" value="1" class="w-5 h-5 rounded border-gray-300 dark:border-gray-600 text-rose-500 focus:ring-rose-500" {{ old('is_active', $package->is_active) ? 'checked' : '' }}>
                <div>
        ...
        <div class="flex flex-row max-sm:flex-col items-center gap-3">
            ...
            <a href="{{ route('packages.show', $package) }}" class="px-6 py-2.5 max-sm:w-full max-sm:py-2 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 text-sm font-medium rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600 transition">
                {{ __('common.cancel') }}
            </a>
        </div>
    </form>
