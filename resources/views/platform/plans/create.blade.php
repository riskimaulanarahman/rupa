...
<div class="mx-auto max-w-4xl space-y-6">
    <section class="flex flex-col gap-3 rounded-2xl border border-gray-200/70 bg-white px-5 py-4 shadow-sm dark:border-gray-700 dark:bg-gray-800 sm:flex-row sm:items-end sm:justify-between">
        <div>
            <h2 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-gray-100">Buat Paket Baru</h2>
            <p class="mt-1 text-sm text-gray-600 dark:text-gray-300">Isi konfigurasi harga dan batasan paket langganan.</p>
        </div>
    ...
    <form action="{{ route('platform.plans.store') }}" method="POST" class="overflow-hidden rounded-2xl border border-gray-200/70 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
        @csrf
        <div class="px-4 py-8 sm:p-8">
            <div class="grid grid-cols-1 gap-6 lg:grid-cols-12">
                ...
                <div class="lg:col-span-4">
                    ...
                    <input type="number" name="max_outlets" id="max_outlets" value="{{ old('max_outlets') }}" placeholder="Kosongkan jika tak terbatas"
                           class="mt-2 block h-12 w-full rounded-lg border border-gray-300 px-4 text-gray-900 shadow-sm focus:border-rose-500 focus:outline-none focus:ring-2 focus:ring-rose-500/20 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100">
                </div>
                ...
                <div class="lg:col-span-8">
                    ...
                    <textarea id="description" name="description" rows="5"
                              class="mt-2 block w-full rounded-lg border border-gray-300 px-4 py-3 text-gray-900 shadow-sm focus:border-rose-500 focus:outline-none focus:ring-2 focus:ring-rose-500/20 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100">{{ old('description') }}</textarea>
                </div>
        ...
        <div class="flex items-center justify-end border-t border-gray-200 px-4 py-4 sm:px-8 dark:border-gray-700">
            <button type="submit" class="rounded-lg bg-rose-600 px-6 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-rose-700">Simpan Paket</button>
        </div>
    </form>
</div>
@endsection
